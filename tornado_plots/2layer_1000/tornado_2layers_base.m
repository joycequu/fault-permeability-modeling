...
% 
base = [0.2, 0.65, 0.65, 65, 600, 2000]; % sc_c
% base = [0.2, 0.65, 0.2, 65, 600, 2000]; % sc_s
% base = [0.65, 0.2, 0.65, 65, 600, 2000]; % cs_c
% base = [0.65, 0.2, 0.2, 65, 600, 2000]; % cs_s
...
vcl       = {[base(1), base(2)], base(3)};   % fraction [-]
dip       = [0, 0];                                                     % [deg.]
faultDip  = base(4);                                           % [deg.]
zf        = [base(5), base(5)];                       % [FW, HW], [m]
zmax      = {repelem(base(6), numel(vcl{1})), repelem(base(6), numel(vcl{2}))};  % {FW, HW}
...
U.flexible        = true;       % default true, much faster but U.coarseDimsfaults_grid_units
...
plotUpscaledPerm(faults, dim, 'all') % only run this line for figures
